...
- Current product checkpoint:
  - **Phase R — Workers & Tools Accountability**
...
- Transaction creation is routed through secure Supabase RPC `create_custody_transaction`.
- Flutter no longer generates official transaction codes for new transactions.
- Transaction `created_by_profile_id` is now derived from authenticated backend context.
- Lost/Damaged approve/reject/settle workflow is routed through secure Supabase RPCs.
- Flutter no longer sends `decidedByProfileId` or `settledByProfileId` for approval workflow actions.
- Signed approval document upload is routed through secure Supabase RPC `upload_transaction_approval_document`.
- Approval document upload accountability is stored through:
  - `approval_document_uploaded_by_profile_id`
  - `approval_document_uploaded_at`
- General transaction editing path is disabled in Flutter Repo/Cubit.
- Transactions should now be changed only through controlled workflows.
...
- Visible search-query state remains synchronized after navigation for:
  ...
  - Custody Balance
  - Tool Summary
  - Auth UX Checkpoint — Email-First Authentication Flow is completed.
- Email Entry route is the unauthenticated entry route.
- Login/Register now receive prefilled locked email from the safe email-first choice step.
- Direct `/login` and `/register` access without a valid email is guarded.
- Logout returns to Email Entry.
- No backend account-existence detection was added; it remains deferred to Phase S.
...
- Flutter uploads the file to Supabase Storage, then the backend writes:
